...
//   where('searchTokens', 'array-contains', state.searchQuery)
// That only works if the ENTIRE typed string (including spaces) is an
// exact stored token — so single words like "gam" matched fine, but any
// multi-word query like "cool gaming" never matched anything, because
// "cool gaming" (with the space) was never stored as one token.
//
// Fix: split the typed query into individual words, then match against
// ANY of those words using 'array-contains-any' (Firestore allows up to
// 10 values). Each word is itself checked as a prefix token, so partial
// words like "gam" still match "gaming", and multi-word queries now work
// because each word is checked independently. No external search
// service is introduced — this still runs entirely on the existing
// searchTokens field.
